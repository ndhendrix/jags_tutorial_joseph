...
# image of the test characteristics priors
...
The model we'll build constructs a 4\*N matrix so we'll first need to use the `var` command to tell JAGS to build that matrix. Below that, we'll use the test results to construct a matrix of true likelihood values, as derived from the test characteristics. We'll also use a vector of 1\*N comprising only 1s, which ensures that our prevalence results are in an appropriate range.
